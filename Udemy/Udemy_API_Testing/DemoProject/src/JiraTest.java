...
//Login Scenario
        ...
        SessionFilter session=new SessionFilter();   // to store authentiction details
                                                    // used with filter(session) in all requests
...
//Add Attachment            using multipart
        ...
        given().header("X-Atlassian-Token","no-check").filter(session).pathParam("key", "10101")
                ...
                .multiPart("file",new File("src/Resource/Jira.txt")).when().
                post("rest/api/2/issue/{key}/attachments").then().log().all().assertThat().statusCode(200);

//Get Issue                 and validate comment in post = comment in get
// pathParam  used to filter request ( specify request)
// queryParam used to filter response
